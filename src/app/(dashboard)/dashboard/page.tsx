...
async function TripDashboardContent({ session }: { session: any }) {
  await connectDB()
  ...
  const myTrips = allTrips.filter(t => t.userId.toString() === session.user.id)
  const sharedTrips = allTrips.filter(t => t.userId.toString() !== session.user.id)
...
function DashboardSkeleton() {
  return (
    <div className="animate-pulse">
      <div className="grid grid-cols-3 gap-4 mb-8">
        <div className="h-28 bg-amber-100/50 rounded-2xl"></div>
        <div className="h-28 bg-amber-100/50 rounded-2xl"></div>
        <div className="h-28 bg-amber-100/50 rounded-2xl"></div>
      </div>
      <div className="h-8 w-48 bg-amber-100/50 rounded mb-4"></div>
      <div className="grid grid-cols-3 gap-4">
        <div className="h-40 bg-amber-100/50 rounded-2xl"></div>
        <div className="h-40 bg-amber-100/50 rounded-2xl"></div>
        <div className="h-40 bg-amber-100/50 rounded-2xl border border-dashed border-amber-200 bg-transparent"></div>
      </div>
    </div>
  )
}
